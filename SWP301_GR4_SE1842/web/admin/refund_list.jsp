                            ...
                            <div class="col-sm-12">
                                <div class="iq-card">
                                    ...
                                    <div class="iq-card-body">
                                        ...
                                        <c:forEach items="${rList}" var="entry">
                                            <c:set var="r" value="${entry.key}" />
                                            <c:set var="status" value="${entry.value}" />
                                            <c:if test="${status == 0}">
                                                <c:set var="border" value="border-danger" />
                                            </c:if>
                                            <c:if test="${status != 0}">
                                                <c:set var="border" value="border-primary" />
                                            </c:if>
                                            <li class="list-all-refund">
                                                <a href="refunds?o=${r.id}">
                                                    <div>
                                                        <div class="timeline-dots ${border}"><i style="color:var(--iq-body-text)" class="ri-pantone-line"></i></div>
                                                        <c:if test="${r.user ne null}">
                                                            <h6 class="float-left mb-1">@${r.user.username}</h6>
                                                        </c:if>
                                                        <c:if test="${r.user eq null}">
                                                            <h6 class="float-left mb-1">${r.cusFullname}</h6>
                                                        </c:if>
                                                        <div class="d-inline-block w-100">
                                                            <p style="color:var(--iq-body-text)">requested to refund (some) book(s) in an order!</p>
                                                        </div>
                                                    </div>
                                                </a>
                                            </li>
