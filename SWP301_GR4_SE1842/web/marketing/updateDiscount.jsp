...
<html>
    <head>
        ...
        <!-- Thêm mã JavaScript để kiểm tra ngày -->
<script>
    // Hàm kiểm tra ngày
    function validateDateRange() {
        var startDate = new Date(document.getElementsByName("fromdate")[0].value);
        var endDate = new Date(document.getElementsByName("todate")[0].value);

        // Kiểm tra nếu có lỗi ngày
        if (isNaN(startDate) || isNaN(endDate)) {
            alert("Invalid date format. Please choose a valid date.");
            return false;
        }
    ...
    // Gắn sự kiện cho nút cập nhật giảm giá để kiểm tra ngày trước khi submit
    document.forms[0].addEventListener("submit", function(event) {
        if (!validateDate()) {
            event.preventDefault(); // Ngăn chặn sự kiện submit nếu có lỗi ngày
        }
    });
</script>
  ...
  <li class="">
            ...
            <aside class="right-side">
                ...
                <section class="content">
                    <div class="row">
                        <h1 style="text-align: center;">Update discount </h1>
                        <form method="post" action="EditDiscount">
                            <div class="col-md-6">
                                <span>ID:</span>
                                <input type="text" class="form-control" name="did" value="${d.discountId}" readonly=""> 
                            </div>
                            <div class="col-md-6">
                                <span>Book:</span>
                                <select class="form-control" name="bookId">
                                    <c:forEach var="b" items="${lb}">
                                        <option ${d.bookId== b.id?"selected":""} value="${b.id}">${b.title}</option>
                                    </c:forEach>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <span>Amount(%):</span>
                                <input type="number" step="0.1" class="form-control" name="amount" value="${d.amount}" required min="0" max="99">
                            </div>
                          
                            <div class="col-md-6">
                                <span>From date:</span>
                                <input type="date" class="form-control" name="fromdate" value="${d.startDate}" onchange="validateDateRange()">
                            </div><!-- comment -->
                            <div class="col-md-6">
                                <span>To Date:</span>
                                <input type="date" class="form-control" name="todate" value="${d.endDate}" onchange="validateDateRange()">
                            </div>
